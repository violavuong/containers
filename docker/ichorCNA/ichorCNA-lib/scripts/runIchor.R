...
option_list <- list(
  ...
  make_option(c("--libdir"), type = "character", default="/ichorCNA/", help = "Script library path. Usually exclude this argument unless custom modifications have been made to the ichorCNA R package code and the user would like to source those R files. Default: [%default]")
)
...
if (genomeBuild == "hg19") {
  centromere <- file.path(paste0(libdir, "inst/extdata/GRCh37.p13_centromere_UCSC-gapTable.txt"))
} else if (genomeBuild == "hg38") {
  centromere <- file.path(paste0(libdir, "inst/extdata/GRCh38.GCA_000001405.2_centromere_acen.txt"))
}
